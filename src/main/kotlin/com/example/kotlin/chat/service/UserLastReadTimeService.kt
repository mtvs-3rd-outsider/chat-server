...
class UserLastReadTimeService(
    ...
    // 메시지 스트림을 받아서 각 메시지에 대해 처리하는 메서드로 수정
    override suspend fun post(inboundMessages: Flow<MessageVM>) {
        inboundMessages.collect { message ->
            ...
            // 각 사용자에 대해 온라인 상태를 체크하고, 필요한 경우 마지막 읽은 시간을 업데이트
            val updatedList = currentList.map { readTime ->
                if (userStatusService.isUserOnline(readTime.userId)) {
                    // 사용자가 온라인 상태일 경우에만 lastReadableTime 업데이트
                    readTime.copy(
                        lastReadableTime = newLastReadableTime
                    )
                } else {
                    readTime // 오프라인일 경우 그대로 유지
                }
            }.toMutableList()

            // 해당 userId의 정보가 없다면 새로 추가
